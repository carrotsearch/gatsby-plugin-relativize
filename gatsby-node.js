...
const relativizeHtmlFiles = async () => {
  ...
  await pMap(paths, async (path) => {
    ...
    contents = contents
      .replace(/\/__RELATIVIZE_PREFIX__\//g, () => relativePrefix);
...
const relativizeJsFiles = async () => {
  ...
  await pMap(paths, async (path) => {
    ...
    contents = contents
      .replace(/["']\/__RELATIVIZE_PREFIX__['"]/g, () => ' __RELATIVIZE_PREFIX__ ')
      .replace(/(["'])\/__RELATIVIZE_PREFIX__\/([^'"]*?)(['"])/g, (matches, g1, g2, g3) => ` __RELATIVIZE_PREFIX__ + ${g1}/${g2}${g3}`);
...
exports.onPreBootstrap = ({ store, reporter }) => {
  ...
  if (!/\/?__RELATIVIZE_PREFIX__/.test(config.pathPrefix)) {
    reporter.panic('The pathPrefix must be set to __RELATIVIZE_PREFIX__ in your gatsby-config.js file');
  }
